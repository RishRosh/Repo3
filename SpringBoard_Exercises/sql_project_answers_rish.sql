...
SELECT count(distinct `name`) CNT FROM `Facilities` WHERE `membercost` = 0
...
SELECT Concat(m.`firstname`, m.`surname`) as Member_Name,
	case when m.`memid` = 0 then f.`guestcost` else f.`membercost` end as cost
FROM (select `bookid`, `memid`, `facid` from `Bookings` WHERE cast(`starttime` as date) = '2012-09-14') b
inner join `Facilities`f on b.`facid` = f.`facid`
inner join `Members` m on b.`memid` = m.`memid`
WHERE case when m.`memid` = 0 then f.`guestcost` else f.`membercost` end > 30
ORDER by 1 DESC
...
SELECT f.`name`,
	sum(case when m.`memid` = 0 then f.`guestcost` else f.`membercost` end) as cost
FROM `Bookings` b
inner join `Facilities`f on b.`facid` = f.`facid`
inner join `Members` m on b.`memid` = m.`memid`
GROUP BY 1
